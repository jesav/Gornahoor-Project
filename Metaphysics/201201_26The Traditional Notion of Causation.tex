...
A cause is a principle on which something else depends for its being. Thus the cause has priority over the effect, not just temporally, but in the wider sense\footnote{\url{https://www.gornahoor.net/?p=3629}} that we give it. Aquinas and Aristotle name four types of causes in two classifications, as shown in this chart:

Classes of causation

\begin{table}[h]
\begin{tabular}{cc}
\textbf{Intrinsic Causes} &
\textbf{Extrinsic Causes}\\
Material &
Efficient\\
Formal &
Final\\
...
\begin{description}
\item[Efficient cause ] That by which the effect is produced. 
\item[Final cause ] That for which the effect is produced. 
\item[Material cause ] That out of which the effect is produced. 
\item[Formal cause ] That which makes the effect to be of a particular kind. 
...
Material Cause $\Rightarrow $ Substantial Cause

Formal Cause $\Rightarrow $ Essential Cause
